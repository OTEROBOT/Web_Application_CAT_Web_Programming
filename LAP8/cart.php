...
if (empty($_SESSION['cart'])) {
    $_SESSION['cart'] = array();  // เคลียร์ค่าในตะกร้าเมื่อไม่มีกระบวนการเพิ่มสินค้า

}

// ตรวจสอบว่า `$ids` ไม่ว่างก่อน Query
$query = (count($productIds) > 0) ? mysqli_query($conn, "SELECT * FROM products WHERE id IN ($ids)") : false;
                            ...
                            <?php if ($rows > 0): ?>
                                <?php while ($product = mysqli_fetch_assoc($query)): ?>
                                    <tr>
                                        ...
                                        <td>
                                            <input type="number" name="product[<?php echo $product['id']; ?>]" value="<?php echo $_SESSION['cart'][$product['id']]; ?>" class="form-control" min="1">
                                        </td>
                                        <td><?php echo number_format($product['price'] * $_SESSION['cart'][$product['id']], 2); ?></td>
                                        <td>
                                            <a onClick="return confirm('Are your sure you want to delete?');" href="<?php echo $base_url; ?>/cart-delete.php?id=<?php echo $product['id']; ?>" class="btn btn-outline-danger">

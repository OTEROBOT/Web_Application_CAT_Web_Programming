...
if(empty($_POST['id'])) {
    $query = mysqli_query($conn, "INSERT INTO products (product_name, price, profile_image, detail) 
VALUES ('$product_name', '$price', '$image_name', '$detail')")
or die('Query failed: ' . mysqli_error($conn));
    ...
    $query = mysqli_query($conn, "UPDATE products SET product_name='{$product_name}', price='{$price}', profile_image='{$image_name}', detail='{$detail}' WHERE id='{$_POST['id']}'")
or die('Query failed: ' . mysqli_error($conn));
}
